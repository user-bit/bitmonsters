<?php class ModeratorsController extends BaseController
{
	...
	public function indexAction()
	{
		...
		if(isset($this->params['delete']) || isset($_POST['delete']))$vars['message']=$this->moderators->delete($this->tb);
		...
		$vars['list']=$this->view->Render('view.phtml',$this->moderators->find(['select'=>'tb.*,tb2.comment as status','join'=>'LEFT JOIN moderators_type tb2 ON tb.type_moderator=tb2.id','where'=>"tb2.id!='1'",'order'=>'tb.`id` DESC','type'=>'rows','paging'=>true]));
	...
	public function editAction()
	{
		...
		$vars['edit']=$this->moderators->find(['select'=>'tb.*,tb2.comment as status','join'=>'LEFT JOIN moderators_type tb2 ON tb.type_moderator=tb2.id','where'=>'__tb.id:='.$this->params['edit'].'__']);
		$vars['types']=$this->db->rows("SELECT * FROM `moderators_type` WHERE id!='1' ORDER BY id ASC");
