<?php class Modules extends Model
{
	...
	function set_module_data($module)
	{
		...
		if(!is_dir(MODULES.$module."/".PathToTemplateAdmin."/data/")) mkdir(MODULES.$module."/".PathToTemplateAdmin."/data/",0755,true);
		$file=MODULES.$module."/".PathToTemplateAdmin."/data/info.txt";
		if(!file_exists($file)) {
			$fp=fopen($file,"w");
			fwrite($fp,"");
			fclose($fp);
		...
		fclose($fd);
		//Translation
		$translate=$this->db->rows("SELECT * FROM translate tb LEFT JOIN ".$this->registry['key_lang_admin']."_translate tb2 ON tb.id=tb2.translate_id WHERE modules_id='{$row['id']}' ORDER BY `key` ASC");
		if(count($translate) != 0) {
			$file=MODULES.$module."/".PathToTemplateAdmin."/data/translate.txt";
			$fp=fopen($file,"w");
			fwrite($fp,"");
			fclose($fp);
			$fd=fopen($file,"w");
		...
		// Configuration
		$config=$this->db->rows("SELECT * FROM config tb WHERE modules_id='{$row['id']}' ORDER BY `name` ASC");
		if(count($config) != 0) {
			$file=MODULES.$module."/".PathToTemplateAdmin."/data/config.txt";
			$fp=fopen($file,"w");
			fwrite($fp,"");
			fclose($fp);
			$fd=fopen($file,"w");
	...
	{
		$message='';
		if(isset($_POST['sub']) && $_POST['module'] != "" && $_POST['sub'] != "") {
			////Добавляем таблицы,если есть db.sql
			$dir=MODULES.$_POST['module']."/".PathToTemplateAdmin."/data/db.sql";//echo $dir;
			$this->addModule($dir);
			$param=array($_POST['name'],$_POST['module'],$_POST['sub'],$_POST['sort']);
			$insert_id=$this->db->insert_id("INSERT INTO `".self::$table."` SET name=?,controller=?,sub=?,sort=?",$param);
			///Права доступа
			...
			if(isset($_POST['get_translate'])) {
				$dir=MODULES.$_POST['module']."/".PathToTemplateAdmin."/data/translate.txt";//echo $dir;
				if(file_exists($dir)) {
					$lines=file($dir);
					$i=0;
					$data=[];
			...
			if(isset($_POST['get_config'])) {
				$dir=MODULES.$_POST['module']."/".PathToTemplateAdmin."/data/config.txt";
				if(file_exists($dir)) {
					$lines=file($dir);
					$i=0;
					$data=[];
			...
			if(isset($_POST['save_id']) && is_array($_POST['save_id'])) {
				if(isset($_POST['save_id'],$_POST['name'])) {
					for ($i=0; $i <= count($_POST['save_id']) - 1; $i++) {
						...
						$this->db->query("UPDATE `".self::$table."` SET `name`=? WHERE id=?",$param);
					}
					$message .= messageAdmin('Данные успешно сохранены');
				} else $message .= messageAdmin('Заполнены не все обязательные поля','error');
			} else {
				if(isset($_POST['name'],$_POST['link'])) {
					$param=array($_POST['name'],$_POST['link'],$_POST['tables'],$_POST['sub'],$_POST['id']);
					$this->db->query("UPDATE `".self::$table."` SET `name`=?,`link`=?,`tables`=?,`sub`=? WHERE id=?",$param);
					/*
					'000'-off;
					'100'-read;
					'200'-read/edit;
	...
	{
		$row=$this->db->row("SELECT tables,controller FROM `".self::$table."` WHERE id=?",[$id]);
		if($row && $row['tables'] != "") {
		...
		if(is_dir($dir) && $row['controller'] != '')Dir::removeDir($dir);
		$this->db->query("DELETE FROM `config` WHERE `modules_id`=?",[$id]);
		$this->db->query("DELETE FROM `translate` WHERE `modules_id`=?",[$id]);
		$this->db->query("DELETE FROM `moderators_permission` WHERE `module_id`=?",[$id]);
