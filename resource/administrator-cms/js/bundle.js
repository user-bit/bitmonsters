 ...
 *
 * @param StringClass name The name of the cookie.
 * @param StringClass value The value of the cookie.
 * @param Object options An object literal containing key/value pairs to provide optional cookie attributes.
 * @option Number|Date expires Either an integer specifying the expiration date from now on in days or a Date object.
 *                             If a negative value is specified (e.g. a date in the past), the cookie will be deleted.
 *                             If set to null or omitted, the cookie will be a session cookie and will not be retained
 *                             when the the browser exits.
 * @option StringClass path The value of the path atribute of the cookie (default: path of page that created the cookie).
 * @option StringClass domain The value of the domain attribute of the cookie (default: domain of page that created the cookie).
...
(function ($, undefined) {
  $.widget("ui.draggable", $.ui.mouse, {
    ...
    _mouseStart: function _mouseStart(event) {
      ...
      this.offsetParentCssPosition = this.offsetParent.css("position"); //The element's absolute position on the page minus margins
    ...
    _convertPositionTo: function _convertPositionTo(d, pos) {
      ...
      var mod = d === "absolute" ? 1 : -1,
          scroll = this.cssPosition === "absolute" && !(this.scrollParent[0] !== document && $.contains(this.scrollParent[0], this.offsetParent[0])) ? this.offsetParent : this.scrollParent; //Cache the scroll
  ...
  $.widget("ui.autocomplete", {
    ...
    _create: function _create() {
      ...
      this.liveRegion = $("<span>", {
        role: "status",
        "aria-live": "polite"
      }).addClass("ui-helper-hidden-accessible").insertBefore(this.element); // turning off autocomplete prevents the browser from remembering the
...
jQuery.tableDnD = {
  ...
  build: function build(options) {
    ...
    this.each(function () {
      ...
      this.tableDnDConfig = jQuery.extend({
        ...
        dragHandle: "move" // If you give the name of a class here, then only Cells with this class will be draggable

      }, options || {}); // Now make the rows draggable

      jQuery.tableDnD.makeDraggable(this);
    }); // Now we need to capture the mouse up and mouse move event
    // We can use bind so that we don't interfere with other event handlers

    jQuery(document).bind('mousemove', jQuery.tableDnD.mousemove).bind('mouseup', jQuery.tableDnD.mouseup); // Don't break the chain

    return this;
  },

  /** This function makes all the rows on the table draggable apart from those marked as "NoDrag" */
  makeDraggable: function makeDraggable(table) {
  ...
  getMouseOffset: function getMouseOffset(target, ev) {
    ev = ev || window.event;
  ...
  mousemove: function mousemove(ev) {
    ...
    var y = mousePos.y - jQuery.tableDnD.mouseOffset.y; //auto scroll the window
    ...
    if (y != jQuery.tableDnD.oldY) {
      ...
      var movingDown = y > jQuery.tableDnD.oldY; // update the old value

      jQuery.tableDnD.oldY = y; // update the style to show we're dragging
  ...
  findDropTargetRow: function findDropTargetRow(draggedRow, y) {
  ...
  mouseup: function mouseup(e) {
    if (jQuery.tableDnD.currentTable && jQuery.tableDnD.dragObject) {
      ...
      var config = jQuery.tableDnD.currentTable.tableDnDConfig; // If we have a dragObject, then we need to release it,
  ...
  serializeTable: function serializeTable(table) {
    var result = "";
  ...
  serializeTables: function serializeTables() {
    var result = "";
    this.each(function () {
      // this is now bound to each matching table
      ...
      buttons: {
        tryAgain: {
          ...
          action: function action() {
            var dataString = 'id=' + this_id + '&table=' + this_table + '&page=' + this_page;
            $.ajax({
              type: "POST",
...
$(document).ready(function () {
  ...
  $(document).on('click', '.add-faq', function () {
    ...
    $.ajax({
      ...
      error: function error() {
        return console.log('otherproduct ERROR');
      }
    });
  });
  $(document).on('click', '.del-faq-item', function (e) {
    e.preventDefault();
    e.stopPropagation();
    var id = $(this).parents('.faq-meta').find('.meta_id').val();
    $.ajax({
      type: "POST",
...
$(document).ready(function () {
  $('.infoblock-select').change(function (e) {
    e.preventDefault();
    e.stopPropagation();
    var this_template = 'path=' + $(this).val();
    $.ajax({
      type: "POST",
...
$(document).ready(function () {
  $("input[name='price_basePrice[]'").bind('change keyup input click', function () {
    bindBasePrice($(this));
  });
  $("input[name='price_discount[]']").bind('change keyup input click', function () {
    bindDiscountPrice($(this));
  });
...
function bindBasePrice($this) {
  ...
  var price = $this.val();
  var ressult = price - price * (discount / 100);
...
$(document).ready(function () {
  ...
  $(document).on("click", ".send_sms_btn", function (e) {
    ...
    var text = $("textarea[name='send_sms']").val();
    var dataString = 'phone=' + phone + '&text=' + text;
    $.ajax({
      type: "POST",
  ...
  $(document).on("click", ".send_sms_ttn_btn", function (e) {
    ...
    var post_ttn = $("input[name='post_ttn']").val();
    console.log(post_ttn);
    var dataString = 'phone=' + phone + '&post_name=' + post_name + '&post_ttn=' + post_ttn;
    $.ajax({
      type: "POST",
...
$(document).ready(function () {
  ...
  $(document).on('click', '.custom-control-label-all', function () {
    if ($('.custom-control-label').prop('checked')) $('.custom-control-label').prop('checked', false);else $('.custom-control-label').prop('checked', true);
  ...
  $('.nav__item').on('click', function () {
    ...
    $(this).addClass('active');
    var open_section = '.tabs-section-' + $(this).data('open');
    $(open_section).removeClass('hidden');
    $.cookie('active_tab_content', open_section);
    $.cookie('active_tab', $(this).data('open'));
  });
  var tab_content = $.cookie('active_tab_content');
  var tab_item = $.cookie('active_tab');
